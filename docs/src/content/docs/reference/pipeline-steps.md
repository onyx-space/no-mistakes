...
- Reruns the pass with the validation error appended when the agent's final JSON fails the schema, up to three attempts, so a formatting slip does not fail a run whose answer was otherwise readable; findings come only from the attempt that validates, and the agent's edits are committed once the attempts settle. Exhausting the attempts commits those edits and still fails the step, so an unreadable answer is never treated as a pass
...
- `pr.instructions` can add the repository's own trusted title and body shape (language, section order, folding) to the drafting prompt; it augments the built-in drafting rules and is read only from the trusted default branch, so a pushed branch cannot reshape the body that reviews it. See [`pr.instructions`](/no-mistakes/reference/repo-config/#prinstructions).
